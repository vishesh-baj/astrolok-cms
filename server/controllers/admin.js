...
exports.chatCallReviewRatingOfAstrologerByUsers = async (req, res) => {
    try {
        ...
        if (rating || review || chatHour || callHour) {
            ...
            let data = await TotalChatCallReviewRatingOfAstrologer.findOne({ Astrologerdetails })
            if (rating) {
                if (!data) {
                    console.log("iamworking");
                    ...
                    for(let i = 0;i< data?.totalRatings?.length ; i++){
                        if (JSON.stringify(data?.totalRatings[i].userdetails) === JSON.stringify(userid)) {
            ...
            if (review) {
                if (!data) {
                    ...
                    for(let i = 0;i< data?.totalReviews?.length ; i++){
                        if (JSON.stringify(data?.totalReviews[i].userdetails) === JSON.stringify(userid)) {
                            // data?.totalRatings[i].rating = rating;
            ...
            if (chatHour) {
                if (!data) {
                    ...
                    for(let i = 0;i< data?.totalChatHours?.length ; i++){
                        if (JSON.stringify(data?.totalChatHours[i].userdetails) === JSON.stringify(userid)) {
                            // data?.totalRatings[i].rating = rating;
            ...
            if (callHour) {
                if (!data) {
                    ...
                    for(let i = 0;i< data?.totalCallHours?.length ; i++){
                        if (JSON.stringify(data?.totalCallHours[i].userdetails) === JSON.stringify(userid)) {
                            // data?.totalRatings[i].rating = rating;
            ...
            return res.status(200).json({
                success:true,
                message: await TotalChatCallReviewRatingOfAstrologer.findOne({ Astrologerdetails })
            })
        }
        else {
            return res.status(404).json({
