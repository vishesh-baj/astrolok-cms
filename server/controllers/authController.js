  ...
  login = async (req, res) => {
    ...
    try {
      ...
      if (!email || !password) {
        ...
        const userExist = await this.authSeriviceInstance.findUserbyEmail(
          email,
          res
        );
        if (userExist) {
        ...
        // as it is not user we will check for astrologer
          ...
          const astrologerExist =
            await this.authSeriviceInstance.findAstrologerByEmail(email, res);
          if (astrologerExist) {
            await this.authSeriviceInstance.login(
              password,
              astrologerExist,
              res
            );
          ...
          const adminExist = await this.authSeriviceInstance.findAdminByEmail(
            email,
            res
          );
          if (adminExist) {
