            ...
            if (userExist) {
                if (!(await bcrypt.compare(password, userExist.password))){
                   return res.status(400).json({
                     ...
                     token = jwt.sign({ id: userExist._id, role: userExist.role }, process.env.SECRET_KEY, { expiresIn: '2h' })
            ...
            if (astroExist) {
                if (!await bcrypt.compare(password, astroExist.password)){
                   return res.status(400).json({
                     ...
                     token = jwt.sign({ id: astroExist._id, role:astroExist.role }, process.env.SECRET_KEY, { expiresIn: '2h' })
