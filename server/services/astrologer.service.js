...
class AstrologerService {
  ...
  async createNewBooking(astrologerId, userId, data) {
    ...
    try {
      ...
      const resp = await AstrologerConsultation.create({
        astrologerId,
        userId,
        bookingtype, bookingdate, bookingtime, status, waitingTime
      })
      resp.save();
      if (resp) {
        return ({
  ...
  async dayFromDate(date) {
    try {
      if (!date) {
        ...
        let dayNames = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]; // an array of strings that contains the names of the days
  ...
  async convertionOfStringTimeToISO(time) {
    try {
      if (!time) {
        ...
        date.setHours(hours, minutes, 0, 0); // set the hours, minutes, seconds and milliseconds of the date object
  ...
  async getServiceScheduleSlots(duration, start, end) {
    try {
      ...
      while (startTime <= endTime) {
        ...
        slot.start = await this.formatTime(startTime); // get the start time as a string in HH:MM format
  ...
  async formatTime(minutes) {
    ...
    return (hours < 10 ? "0" : "") + hours + ":" + (minutes < 10 ? "0" : "") + minutes; // pad with zeros and return as a string
  }
